...
<table class="table bg-white rounded shadow-sm  table-hover">
                ...
                <div class="row my-5">
                    ...
                    include "connect1.php";
                    $sql2="select ID from joined_in where EMAIL='$pemd';";
                    $result2=mysqli_query($con,$sql2);
                     if(mysqli_num_rows($result2) > 4){
                        echo '<button  style="background-color:green" 
                        class="btn btn-primary"><a style="text-decoration:none" 
                        href="userstatus.php?&email='.$pemd.'#foo"class="text-light">Get Premium</a></button>';

                     }
                     else{
                        include "connect1.php";
                         $sql1="select ID from joined_in where TOKEN_SYMBOL='$token_symbol' and EMAIL='$pemd';";
                         $result1=mysqli_query($con,$sql1);
                         if(mysqli_num_rows($result1) > 0){
                             echo '<button  style="background-color:green" 
                             class="btn btn-primary"><a style="text-decoration:none" 
                             href="#"class="text-light">ALREADY CLAIMED</a></button>';
                            }else{
                                echo '<button style="style="background-color:black"  class="btn btn-danger" onclick="myFunction()"><a style="text-decoration:none"  href="join.php?tokensymbol='.$token_symbol.'&email='.$pemd.'"
                                class="text-light">Claim</a></button> ';
